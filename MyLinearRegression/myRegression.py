...
class MyRegression:
    def __init__(self, df, X, y, test_size=0.2, random_state=42,
                 poly_degree=2, lasso_alpha=0.1, ridge_alpha=0.1):
    ...
    def predict(self):
        ...
        self.poly_pred = self.poly_pipeline.predict(self.X_test)

    def calculate_metrics(self):
        mse = mean_squared_error(self.y_test, self.y_pred)
        rmse = np.sqrt(mse)
        metrics = {
            "linear_r2": r2_score(self.y_test, self.y_pred),
            "lasso_r2": r2_score(self.y_test, self.lasso_pred),
            "ridge_r2": r2_score(self.y_test, self.ridge_pred),
            "poly_r2": r2_score(self.y_test, self.poly_pred),
            "mse": mse,
            "rmse": rmse,
        }
        bias, variance = self._calculate_bias_variance()
        metrics["bias"], metrics["variance"] = bias, variance
        return metrics

    def _calculate_bias_variance(self, n_bootstraps=50):
        predictions = []
