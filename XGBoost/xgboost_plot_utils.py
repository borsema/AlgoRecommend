...
def plot_xgboost_boundaries(X, y, top_features, params, class_names=None, max_points=1000):
    ...
    for f1, f2 in list(combinations(top_features, 2))[:6]:
        ...
        ax.set_xlabel(f1)
        ax.set_ylabel(f2)
        ax.set_title(f"XGBoost Boundary: {f1} vs {f2}")
        ax.legend(title="Classes")
        figs.append(fig)

    return figs

def plot_xgboost_surfaces(X, y_reg, top_features, params, max_points=1000):
    ...
    for f1, f2 in list(combinations(top_features, 2))[:6]:
        ...
        reg = XGBRegressor(**params, random_state=42, eval_metric='rmse')
        ...
        ax.set_xlabel(f1)
        ax.set_ylabel(f2)
        ax.set_title(f"XGBoost Surface: {f1} vs {f2}")
        figs.append(fig)
