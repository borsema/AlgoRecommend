...
def plot_svm_boundaries(X, y, top_features, params, class_names=None, max_points=1000):
    ...
    for f1, f2 in list(combinations(top_features, 2))[:6]:
        ...
        ax.set_xlabel(f1)
        ax.set_ylabel(f2)
        ax.set_title(f"SVM Boundary: {f1} vs {f2}")
        ax.legend(title="Classes")
        figs.append(fig)

    return figs

def plot_svm_surfaces(X, y_reg, top_features, params, max_points=1000):
    ...
    for f1, f2 in list(combinations(top_features, 2))[:6]:
        ...
        ax.set_xlabel(f1)
        ax.set_ylabel(f2)
        ax.set_title(f"SVM Surface: {f1} vs {f2}")
        figs.append(fig)
